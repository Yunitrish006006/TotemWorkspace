...
This audit reviews all 32 curated contracts against the active Totem repositories and qualifies which relationships may safely retarget from module nodes to feature nodes in the V2 viewer.
...
Core explicitly defines `DeathBackpackCreatedEvent` as published after Remnant commits a death backpack and defines recovery lifecycle semantics. Current Remnant repository code search did not independently locate the publisher call during this audit, so the viewer labels the implementation status `contract-defined` rather than claiming freshly verified publisher evidence.
...
Provider registration contracts remain explicit architecture facts and are kept separate from the five runtime relay contracts above.

1. `observer:automata@1` — valid: `automata_copper_golem@1`.
2. `observer:locksmith@1` — valid: `locksmith_management@1`.
3. `observer:nexus@3` — **corrected from `observer:nexus@2`**; active provider reports protocol 3 and eight variants.
4. `observer:nexus-death@1` — valid: `nexus_death_node_admin@1`; owns death-node administration semantics separately from generic Nexus Observer.
5. `observer:remnant@1` — valid: `remnant_backpack@1`.
6. `observer:villagers@1` — valid: `villagers_woodcutter@1`.
