...
renderer = Path('viewer/graph-v2-cluster-v2.js')
s = renderer.read_text()

marker = '  function modulePosition(module, peripheralIndex, peripheralCount, radius) {'
if 'function relationAwareScatter' not in s:
    helper = r'''  function vecLength(v) {
...
'''
    if marker not in s:
        raise SystemExit('modulePosition marker missing')
    s = s.replace(marker, helper + marker)

replacements = {
    'var position = scatter(parent, feature.id, "feature", radius);': 'var position = relationAwareScatter(parent, feature.id, "feature", radius, moduleId, nodes, null);',
    'var position = scatter(parent, id, "capability", radius);': 'var position = relationAwareScatter(parent, id, "capability", radius, moduleId, nodes, [{ targetId: capability.providerModuleId, weight: relationWeight("shared-capability"), kinds: ["shared-capability"] }]);',
    'var position = scatter(parent, category.id, "category", radius);': 'var position = relationAwareScatter(parent, category.id, "category", radius, moduleId, nodes, []);'
}
for old, new in replacements.items():
    if old in s:
        s = s.replace(old, new)
    elif new not in s:
        raise SystemExit(f'missing renderer marker: {old}')

if 'relationAwareScatter: relationAwareScatter' not in s:
    s = s.replace('    scatter: scatter,', '    scatter: scatter,\n    featureRelations: featureRelations,\n    relationAwareScatter: relationAwareScatter,')

renderer.write_text(s)

validator = Path('scripts/validate-3d-cluster.mjs')
v = validator.read_text()
if '"function relationAwareScatter"' not in v:
    v = v.replace('  "function scatter",', '  "function scatter",\n  "function featureRelations",\n  "function relationAwareScatter",')
anchor = 'assert.ok(source.includes("var moduleRadius = moduleOrbitRadius(expandedCount)"), "scene layout must use the dynamic module orbit radius");\n'
checks = r'''
...
'''
if 'Relation-aware layout regression' not in v:
    if anchor not in v:
        raise SystemExit('validator anchor missing')
    v = v.replace(anchor, anchor + checks)
v = v.replace('deterministic clusters, dynamic spacing,', 'deterministic relation-aware clusters, dynamic spacing,')
