...
check(agentInstructions.includes("from Web, Flutter, legacy Viewer, Discord, Bridge, CLI, IDE, or sibling repository Codex"), "All surfaces and sibling repositories must share the intelligence lifecycle");
check(agentInstructions.includes("same normalized task, semantic focus and workspace state must yield equivalent constraints"), "Starting surface must not change execution constraints");
check(agentInstructions.includes("does not prescribe the internal agent topology"), "Astra must retain topology freedom");
check(agentInstructions.includes("max concurrent writes") && agentInstructions.includes("shared-contract stabilization"), "Concurrency and shared contract ordering remain mandatory");
check(agentInstructions.includes("actual validation") && agentInstructions.includes("independentReviewRequired"), "Deterministic validation and required independent review cannot be omitted");
