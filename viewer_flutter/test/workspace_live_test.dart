...
void main() {
  test(
    'execution constraints do not synthesize agents from historical plans',
    () {
      final historical = OrchestrationSummary.fromJson(<String, dynamic>{
        'mode': 'guarded-parallel',
        'assignments': <Object>[
          <String, Object>{'role': 'worker'},
        ],
      });
      expect(historical.waves, isEmpty);
      expect(historical.maxConcurrentWrites, 0);
      final current = OrchestrationPlan.fromJson(<String, dynamic>{
        'schemaVersion': 2,
        'affectedModules': <String>['totem-core'],
        'execution': <String, Object>{
          'maxConcurrentWrites': 1,
          'sharedContractStabilizationRequired': true,
        },
        ...
        'waves': <Object>[
          <String, Object>{
            'id': 'verification',
            'writeAllowed': false,
            'modelHint': 'lightweight-preferred',
            'dependsOn': <String>['implementation'],
          },
        ],
      });
      expect(current.schemaVersion, 2);
      expect(current.modules, <String>['totem-core']);
      expect(current.summary.independentReviewRequired, isTrue);
      expect(current.summary.sharedContractStabilizationRequired, isTrue);
      expect(current.waves.single.dependsOn, <String>['implementation']);
      expect(current.waves.single.writeAllowed, isFalse);
    },
  );
  test(
    'published TotemWorkspace Pages discover the loopback workspace API',
    () {
      expect(
      ...
      expect(
        discoverLocalApiBase(
          pageUri: Uri.parse(
            'https://yunitrish006006.github.io/TotemWorkspace/legacy/',
          ),
        ),
        'http://127.0.0.1:18765',
      );
  ...
  test('Flutter dev server discovers the loopback workspace API', () {
    ...
    expect(
      discoverLocalApiBase(
        pageUri: Uri.parse('http://127.0.0.1:18765/flutter/'),
      ),
      'http://127.0.0.1:18765',
    );
  });
  ...
  test('workspace status parses dirty, drift, and missing counts', () {
        ...
        <String, Object>{
          ...
          'recentChanges': <String, Object>{
            ...
            'files': <Object>[
              <String, Object>{
                'status': 'A',
                'path': 'src/main/resources/assets/totem/lang/ja_jp.json',
              },
            ],
          },
    ...
    expect(
      status.module('totem-a')?.recentChanges?.files.single.path,
      'src/main/resources/assets/totem/lang/ja_jp.json',
    );
    expect(status.module('totem-a')?.branch, 'main');
  });

  test(
    'local client reads graph/status and sends incremental refresh request',
    () async {
      ...
      final mock = MockClient((request) async {
        ...
        if (request.url.path == '/api/health') {
          return http.Response(
            jsonEncode(<String, Object>{'status': 'ok', 'mode': 'local'}),
            200,
          );
        }
        ...
        if (request.url.path == '/api/replay/frame') {
          return http.Response(
            jsonEncode(<String, Object?>{
              ...
              'generatedAt': 'now',
              'sequence':
                  int.tryParse(request.url.queryParameters['sequence'] ?? '') ??
                  5,
              'latestSequence': 9,
              'live': false,
        ...
        if (request.url.path == '/api/orchestration-plan') {
          return http.Response(
            jsonEncode(<String, Object?>{
              ...
              'affectedModules': <String>['totem-core', 'totem-alchemy'],
              'waves': <Object>[
                <String, Object>{
                  'id': 'discovery',
                  'modelHint': 'lightweight-preferred',
                ...
                <String, Object>{
                  'id': 'shared-contract',
                  'modelHint': 'strong-reasoning-preferred',
                  'modules': <String>['totem-core'],
      ...
      final updated = await client.updateViewerSettings(
        settings.copyWith(promptEnabled: true),
      );
      ...
      expect(adapter.currentTask?.orchestration?.parallelismAllowed, isTrue);
      expect(adapter.currentTask?.orchestration?.maxConcurrentWrites, 1);

      final orchestration = await client.orchestrationPlan(
        'inspect outline api',
        moduleId: 'totem-core',
      );
      expect(orchestration.summary.parallelismAllowed, isTrue);
      expect(orchestration.score, 7);
      expect(orchestration.waves.length, 2);
      expect(orchestration.waves.last.writeAllowed, isTrue);
      expect(orchestration.waves.last.modelHint, 'strong-reasoning-preferred');
      ...
      expect(
        replayTimeline.sessions.single.milestones.single.type,
        'commit_created',
      );
      ...
      expect(
        replayFrame.changeIntelligence?.changedEntityIds,
        contains('component:totem-core:api'),
      );
      ...
      expect(
        promptSubmission.orchestration?.summary.parallelismAllowed,
        isTrue,
      );
      expect(promptSubmission.orchestration?.waves.length, 2);
      ...
      final refreshChange = await client.refresh(
        modules: const <String>['totem-core'],
      );
      ...
      final settingsUpdate = requests.singleWhere(
        (request) =>
            request.url.path == '/api/viewer-settings' &&
            request.method == 'POST',
      );
      expect(
        (jsonDecode(settingsUpdate.body)
            as Map<String, dynamic>)['promptEnabled'],
        isTrue,
      );

      final activityRequest = requests.singleWhere(
        (request) => request.url.path == '/api/activity',
      );
      ...
      expect(
        (jsonDecode(orchestrationRequest.body)
            as Map<String, dynamic>)['query'],
        'inspect outline api',
      );

      final promptRequest = requests.singleWhere(
        (request) => request.url.path == '/api/prompt',
      );
      expect(
        (jsonDecode(promptRequest.body) as Map<String, dynamic>)['prompt'],
        'inspect outline api',
      );

      final replayRequest = requests.singleWhere(
        (request) => request.url.path == '/api/replay',
      );
      expect(replayRequest.method, 'GET');

      final replayFrameRequest = requests.singleWhere(
        (request) => request.url.path == '/api/replay/frame',
      );
      expect(replayFrameRequest.url.queryParameters['sequence'], '5');

      final refresh = requests.singleWhere(
        (request) => request.url.path == '/api/refresh',
      );
      expect(refresh.method, 'POST');
      expect(jsonDecode(refresh.body), const <String, Object>{
        'modules': <String>['totem-core'],
      });
      client.close();
    },
  ...
  test(
    'local client reads the private conversation transcript and publishes a debounced draft',
    () async {
      final requests = <http.Request>[];
      final mock = MockClient((request) async {
        requests.add(request);
        ...
        if (request.url.path == '/api/conversation/draft') {
          return http.Response(
            jsonEncode(<String, Object>{'status': 'accepted'}),
            202,
          );
        }
        return http.Response('not found', 404);
      ...
      final draftRequest = requests.singleWhere(
        (request) => request.url.path == '/api/conversation/draft',
      );
      expect(draftRequest.method, 'POST');
      expect(jsonDecode(draftRequest.body), const <String, Object>{
        'clientId': 'viewer:this',
        'text': '網頁未送出草稿',
      });
      client.close();
    },
